...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('tetris.sidebarView', provider)
    );

    // 允许通过命令重置游戏（可选）
    context.subscriptions.push(
        vscode.commands.registerCommand('tetris.reset', () => {
            provider.resetGame();
        })
    );
}

class TetrisViewProvider implements vscode.WebviewViewProvider {
    ...
    public resolveWebviewView(
        ...
        const dropSpeed = config.get<number>('dropSpeed') || 1000;
    ...
    private _getHtmlForWebview(webview: vscode.Webview, dropSpeed: number) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
